...
def pi_n(n, theta):
    if n == 0:
        ...
        pi=(2*n-1)/(n-1) * np.cos(theta) * pi_n(n-1, theta) - n/(n-1)*pi_n(n-2,theta)
    return  pi
...
def tau_n(n, theta):
    ...
    if n==1:
        ...
        tau = n * np.cos(theta) * pi_n(n, theta) - (n+1)*pi_n(n-1,theta)
    return tau 
...
def matriz_stokes_mie(l, wl, r, theta, phi, px, py):
    ...
    l=1
    gmp, gmm, gep, gem = BSC_plana(l, px, py)
...
A11 = 3*np.abs(E0)**2*np.sin(theta)**4/(8*np.pi*(k*r)**2)

U[0][0] = 1+np.cos(theta)**2
U[0][1] = -np.sin(theta)**2
U[0][2] = 0
U[0][3] = 2*np.cos(theta)

U[1][0] = 1+np.cos(theta)**2
U[1][1] = np.sin(theta)**2
U[1][2] = 0
U[1][3] = 2*np.cos(theta)

U[2][0] = 0
U[2][1] = 0
U[2][2] = np.sin(theta)**2
U[2][3] = 0

U[3][0] = 2*np.cos(theta)
U[3][1] = 0
U[3][2] = 0
U[3][3] = 1+np.cos(theta)**2
