...
def X_lm(l, m, Theta, Phi):
    ...
    X_theta = pre * (1*m/np.sin(Theta)) * Y
    X_phi   = pre * -1j * dY_t
...
l_max = 50               # orden máximo
nr = 150               # puntos en r
nphi = 250             # puntos en phi

# --- CONFIGURACIÓN PARA PLANO XY SOLAMENTE ---
ntheta = 1              # Solo 1 punto en theta
r = np.linspace(1e-6, 5*wl, nr)
theta = np.array([np.pi/2])  # Theta fijo a 90 grados (Plano XY)
...
#%% Cálculo de los campos (suma de modos)
...
print("Calculando campos en plano XY...")
...
#E_plot = np.squeeze(np.sqrt(np.abs(Er)**2+np.abs(Etheta)**2+np.abs(Ephi)**2))
...
plt.xlabel('x')
plt.ylabel('y')
plt.title(f'E total en Plano XY (l_max={l_max})')
plt.axis('equal')
plt.show()
...
plt.xlabel('x')
plt.ylabel('y')
plt.title(f'H total en Plano XY (l_max={l_max})')
plt.axis('equal')
plt.show()
